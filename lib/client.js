 ...
 * entry (scope any), persisted view preferences (localStorage), and
 * "show more" pagination for large result sets.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			".dap-tab{position:absolute;right:0;bottom:0;z-index:30;display:inline-flex;align-items:center;gap:6px;height:30px;padding:0 14px;border:none;border-top:1px solid var(--dsw-alias-border-l1,#ececec);border-left:1px solid var(--dsw-alias-border-l1,#ececec);border-radius:10px 0 0 0;background:var(--dsw-alias-bg-base,#fff);color:var(--dsw-alias-label-secondary,#555);font:inherit;font-size:12px;line-height:20px;cursor:pointer;pointer-events:auto;transition:color .1s,background-color .1s}",
			".dap-tab:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.05));color:var(--dsw-alias-label-primary,#1a1a1a)}",
			".dap-tab:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary,#4f7cff);outline-offset:-2px}",
		...
		/**
		 * True while the details column is open. The panel subtree stays mounted
		 * even when the column is collapsed to width 0, so the open state is read
		 * off the shell frame's `data-details-collapsed` attribute (the overlay
		 * layer the trigger renders in is a child of that frame).
		 * @returns whether the details column is open.
		 */
		...
		/**
		 * Window-chrome trigger: a flat tab docked flush against the window's
		 * bottom-right corner (no shadow, square bottom corners, window border
		 * tokens), so it reads as part of the shell frame rather than a floating
		 * button. Hidden while the details column is open.
		 * @param props - the injected openDetails action and the locale seat t.
		 * @returns a docked tab or null while the details column is open.
		 */
